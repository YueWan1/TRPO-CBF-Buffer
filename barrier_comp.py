...
class BARRIER():
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope('Compensator'):
            #Input will be observation
            self.x = tf.compat.v1.placeholder(tf.float32, [None, self.input_size], name='Obs')
            #Target will be control action
            self.target = tf.compat.v1.placeholder(tf.float32, [None, self.action_size], name='Target_comp')
        ...
        tr_vrbs = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='Compensator')
        for i in tr_vrbs:
            print(i.op.name)
