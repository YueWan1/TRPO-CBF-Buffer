...
class Car():
    ...
    def nextStep(self,act,act_noise):
        ...
        self.vel = float(self.vel + accel*self.dt)
        self.pos = float(0.5*accel*self.dt**2 + self.vel*self.dt + self.pos)
...
class allCars():
    ...
    def getAllStates(self):
        # print("Car 1 state:", self.car_1.getState())
        # print("Car 2 state:", self.car_2.getState())
        # print("Car 3 state:", self.car_3.getState())
        # print("Car 4 state:", self.car_4.getState())
        # print("Car 5 state:", self.car_5.getState())
    ...
    def get_safe_action(self, obs):
        """
        Returns a safe action for the ego car based on the current state.
        ...
        distance_to_front_car = self.env.car_4.frontCar.pos - position
        if distance_to_front_car < 5.0:
            return np.array([max(0, velocity - 5)])  # Decelerate to maintain safe distance
        return np.array([velocity])  # Maintain current velocity if safe
