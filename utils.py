...
def LINEAR(x, hidden, name='None'):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        weight = tf.get_variable('Weight', [x.get_shape()[-1], hidden], initializer=tf.truncated_normal_initializer(stddev=0.02), use_resource=True)  # Near empty NN
        ...
        bias = tf.get_variable('Bias', [hidden,], initializer=tf.truncated_normal_initializer(stddev=0.1), use_resource=True)  # Near empty NN    
...
def CONJUGATE_GRADIENT(fvp, y, k=10, tolerance=1e-6):
    ...
    for i in range(k):
        ...
        p_dot_FIM_p = p.dot(FIM_p)

        # Log the key values
        print(f"Iteration {i + 1}:")
        print(f"  r_transpose_r = {r_transpose_r:.6f}")
        print(f"  p.dot(FIM_p) = {p_dot_FIM_p:.6f}")

        # Ensure p.dot(FIM_p) is not zero or negative
        ## if p_dot_FIM_p <= 0:
        ##    print(f"Warning: p.dot(FIM_p) <= 0 at iteration {i + 1}. Exiting CG.")
        ##    break
        
        alpha_k = r_transpose_r / p.dot(FIM_p)
        print(f"  alpha_k = {alpha_k:.6f}")
...
def is_safe(state, min_distance=3.0):
    """
    General function to check if an action is safe given the current state.
    
    Parameters:
    - state: The current state of the environment (positions, velocities, etc.).
    - min_distance: Minimum safe distance between entities (e.g., cars).
...
def sample_batch(BufS, BufC, batch_size):
    """
    Samples a minibatch of transitions from safe and collision buffers.

    Parameters:
    - BufS (list): Safe buffer containing safe transitions.
    - BufC (list): Collision buffer containing unsafe transitions.
    - batch_size (int): Total number of samples to draw.

    Returns:
    - list: Minibatch of transitions.
    """
    # Determine the number of samples from each buffer
    #num_safe_samples = int(batch_size * safe_fraction)
    #num_collision_samples = batch_size - num_safe_samples

    # Sample from buffers
    #safe_samples = random.sample(BufS, min(len(BufS), num_safe_samples))
    #collision_samples = random.sample(BufC, min(len(BufC), num_collision_samples))
    
    combined = BufS + BufC
    if len(combined) < batch_size:
        return combined  # Return all transitions if not enough for batch
    return random.sample(combined, batch_size)

    # Combine and shuffle samples
    minibatch = safe_samples + collision_samples
