...
class TRPO():
    ...
    def train(self, iteration):
        ...
        ### Sample minibatches from BufS and BufC
        minibatch = sample_batch(self.BufS, self.BufC, self.args.timesteps_per_batch)
        for (s, a, s_next, r) in minibatch:
            if (tuple(s), tuple(a), None, r) in self.BufC:
                target = r  # Immediate collision reward
            else:
                dummy_path = {
                    "Observation": np.array([s_next]),  # Single observation (state)
                    "Reward": np.array([r], dtype=np.float32),           # Single reward
                    "Done": np.array([0], dtype=np.int32)              # Not done (assume continuing episode)
                }
                advantage_estimate = self.gae.get_advantage([dummy_path])
                target = r + self.args.gamma * advantage_estimate[0]
        ...
        feed_dict = {self.obs: np.array(s).reshape(1, -1), self.action: np.array(a).reshape(-1, 1), self.advantage: target.reshape(-1), self.old_action_dist_mu:np.expand_dims(action_dist_mu, axis=1), self.old_action_dist_logstd:np.expand_dims(action_dist_logstd, axis=1)}
        ...
        #fisher_vector_product = regularized_fvp(fisher_vector_product, regularization_coeff=1e-6)
        #policy_g = policy_g / np.linalg.norm(policy_g)  # Scale gradients to unit norm
    ...
    def build_policy(self, states, name='Policy'):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            action_dist_logstd = tf.get_variable('logstd', initializer=tf.constant_initializer(0), shape=[1, self.action_size], use_resource=True)
    ...
    def rollout(self):
        ...
        while timesteps < self.args.timesteps_per_batch:
            ...
            for i in range(self.args.max_path_length):
                ...
                ### Check safety and modify action if necessary
                safe_action = cbf.control_barrier(self, np.squeeze(prev_obs), action_dist_mu_RL, f, g, x, std)
                if not is_safe(prev_obs):
                    self.BufC.append((tuple(prev_obs), tuple(safe_action), None, self.args.bar_constraint_max))
                    ### safe_action = self.env.get_safe_action(prev_obs)
                else:
                    self.BufS.append((tuple(prev_obs), tuple(safe_action), tuple(next_obs), reward_))
